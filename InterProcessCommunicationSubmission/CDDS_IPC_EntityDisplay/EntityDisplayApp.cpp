...
/*gets the count of entities from the first dummy entity object in the virtual file which holds it.
  Also opens the virtual file for use. Returns false if fails.*/
bool getEntityCount(HANDLE& fileHandle, int& count)
{
	...
	{
		std::cout << std::endl << "Could not open file mapping object for reading entities. Error code: " << GetLastError() << std::endl << std::endl;
		return false;
	}
	...
	{
		std::cout << std::endl << "Could not get entity count from file mapping object! Error code: " << GetLastError() << std::endl << std::endl;
		return false;
	}

	count = (int)*floatHandle;

	std::cout << std::endl << std::string("Got entity count! Value: ") + std::to_string(count) << std::endl << std::endl;
...
{
	if (!getEntityCount(fileHandle, entityCount))
	{
		return false;
	}
	...
	{
		std::cout << std::endl << "Could not view file mapping object to read entities! Error code: " << GetLastError() << std::endl << std::endl;
		return false;
	}

	std::cout << "Successfully opened file mapping object to read entities!" << std::endl << std::endl;
