...
{
	//assign dummy entity count entity to position 0
	...
	{
		std::cout << std::endl << "Could not create file mapping object! Error code: " << GetLastError() << std::endl << std::endl;
		return false;
	}

	std::cout << std::endl << "Successfully created file mapping object!" << std::endl << std::endl;

	dataHandle = (CombinedData*)MapViewOfFile(fileHandle, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(CombinedData));

	if (dataHandle == nullptr)
	{
		std::cout << std::endl << "Could not open and initialize mapped file object! Error code: " << GetLastError() << std::endl << std::endl;
		return false;
	}

	std::cout << std::endl << "Successfully wrote initial data to file mapping object!" << std::endl << std::endl;

	updateNSM();//fill virtual file with entities
...
void EntityEditorApp::closeEntityNSM()
{
	...
	std::cout << std::endl << "Closed Virtual file!" << std::endl << std::endl;
}
